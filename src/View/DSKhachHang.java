...
import Service.impl.ViewKhachHangServiceImpl;
...
public class DSKhachHang extends javax.swing.JPanel {
    ...
    private ViewKhachHangService vkhs = new ViewKhachHangServiceImpl();
    private List<KhachHang> listKhachHang = new ArrayList<>();

    public DSKhachHang() {
        initComponents();
        tbHienThi.setModel(dtm);
        String[] header = {"STT","Họ tên", "Ngày sinh", "Giới tính", "SDT", "Email", "Địa chỉ", "Người tạo", "Ngày tạo"};
        dtm.setColumnIdentifiers(header);
        listKhachHang = vkhs.getAll();
        showDataTbale(listKhachHang);
    }
    ...
    private void lbNextMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lbNextMouseClicked
        // TODO add your handling code here:
        if (tbHienThi.getSelectedRow() > -1 && tbHienThi.getSelectedRow() < listKhachHang.size()) {
            int row = tbHienThi.getSelectedRow();
            tbHienThi.setRowSelectionInterval(row + 1, row + 1);
        }
    }//GEN-LAST:event_lbNextMouseClicked

    private void showDataTbale(List<KhachHang> list) {
        dtm.setRowCount(0);
        int i = 1;
        for (KhachHang kh: list) {
            dtm.addRow(kh.toDataRow(i));
            i++;
        }
    }
